...
uintptr_t pk_vm_init()
{
  ...
  __map_kernel_range(PAYLOAD_ADDR, PAYLOAD_ADDR, first_free_paddr - PAYLOAD_ADDR, PROT_READ|PROT_WRITE|PROT_EXEC);

  current.mmap_max = current.brk_max =
    MIN(PAYLOAD_ADDR, mem_size - (first_free_paddr - PAYLOAD_ADDR));
