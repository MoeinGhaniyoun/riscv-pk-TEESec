...
void handle_trap(trapframe_t* tf)
{
  ...
  if (tf->cause == CAUSE_MISALIGNED_FETCH){
  	handle_misaligned_fetch(tf);
  }
  else if(tf->cause == CAUSE_FETCH_PAGE_FAULT){
  	handle_fault_fetch(tf);
  }
  ...
  else if(tf->cause == CAUSE_BREAKPOINT){
  	...
  	handle_misaligned_store(tf);	
        ...
        handle_fault_load(tf);
  }
  else if (tf->cause == CAUSE_STORE_PAGE_FAULT){
        handle_fault_store(tf);
  }
